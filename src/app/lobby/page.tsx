  ...
  useEffect(() => {
    ...
    const checkForInvitation = async () => {
      try {
        ...
        if (response.state) {
          router.push(
            `/play?white_player=${response.state!.whitePlayer}&black_player=${response.state!.blackPlayer}`
          );
        }
      } catch (e) {
  ...
  return (
    <main className="flex flex-col items-center justify-center min-h-screen bg-zinc-900">
      <Card className="p-10 bg-zinc-950 shadow-lg rounded-lg max-w-md w-full">
        ...
        <p className="text-[10px] text-center indent-0 mr-2 pb-5 mr-4 -mt-2 -ml-2">
          {publicKeyString}
        </p>
        ...
        />
        <Spacer y={1} />
        <Button className="w-full bg-zinc-800" onClick={handleStartGame}>
          Start a Game
        </Button>
      </Card>
    </main>
